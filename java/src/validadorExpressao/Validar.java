...
public class Validar {
	...
	public void caracterInvalido() throws Exception {
		...
		for (int x = 0; x < palavra.length; x++) {
			if (!palavra[x].contains("(") && !palavra[x].contains("~") && !palavra[x].contains("^")
					&& !palavra[x].contains("v") && !palavra[x].contains("-") && !palavra[x].contains("<")
					&& !palavra[x].contains(")") && !palavra[x].contains("T") && !palavra[x].contains("F")) {
		...
		while (quebrador.hasMoreTokens() == true) {
			...
			for (int x = 0; x < palavraQuebrada.length; x++) {
				...
				if (quebrador.hasMoreTokens() == false && x == (palavraQuebrada.length - 1)) {
					...
					// Variavel tem que ser 1 para ser valida, isso significa que nenhum operador ou
					// negacao esta terminando a frase.
					if (variavel != 1) {
						return false;
					} else {
						variavel--;
